...
class RaceTrack:
    ...
    def read_distances(self, x, y, max_sensor_range, other_car_pos=None, noisy=False, std=0):
        """
        ...
        if other_car_pos is not None:
            other_x, other_y = other_car_pos
            car_length, car_width = 40, 20
            directions = [(0, -1), (0, 1), (-1, 0), (1, 0)]  # up, down, left, right
            for idx, (dx, dy) in enumerate(directions):
                dist = 0
                while dist < readings[idx]:
                    check_x = x + dx * dist
                    check_y = y + dy * dist
                    # Check if the sensor ray intersects with the other car's bounding box
                    if (abs(check_x - other_x) < car_length / 2 and 
                        abs(check_y - other_y) < car_width / 2):
                        readings[idx] = dist
                        break
                    dist += 1
